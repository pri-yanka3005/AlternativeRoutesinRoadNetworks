...
struct vehicles
{ // stores the state of each vehicles.
...
void speed_shuffle()
{ // for shuffling the speed of the vehicles.
    double d = 0.005;
...
{ // for assigning the directions and orientations to the vehicles.
    srand(time(0));
    for (int i = 1; i <= no_of_vehicle; i++)
    {
        if (i <= 8)
...
void color_shuffle()
{ // for shuffling the color of the vehicles.
    srand(time(0));
...
{ // for assigning the positions to the vehicles.
    srand(time(0));
    int j;
    char k;
...
void draw_path()
{ // for displaying the shortest path to be followed
    glColor3f(1.0, 0.0, 0.0);
...
void pointMap()
{ // for mapping points to the coordinates.
...
void find_weights()
{ // for finding the weights across each road
    int i, j, k, x, y;
    srand(0);
    for (i = 1; i <= m; i++)
    {
